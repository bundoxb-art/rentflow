...
export default function AuthPage() {
  ...
  const submit = async () => {
    ...
    if (mode === "signup") {
      ...
      if (!form.password || form.password.length < 6) { setError("Password must be at least 6 characters"); setLoading(false); return; }
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="space-y-4">
            ...
            <button onClick={submit} disabled={loading}
              ...
              ) : mode === "login" ? `Log In as ${role === "landlord" ? "Landlord 🏢" : "Tenant 🏠"} →` : "Create Account & Continue →"}
            </button>
