...
export default function ManagerRegister() {
  ...
  const register = async () => {
    ...
    if (!form.password) { setError("Please create a password"); setLoading(false); return; }
    if (form.password.length < 8) { setError("Password must be at least 8 characters"); setLoading(false); return; }
    if (form.password !== form.confirm) { setError("Passwords do not match"); setLoading(false); return; }

    // Check if email already registered as manager
    ...
    if (data.user) {
      ...
      await supabase.from('audit_logs').insert({
        ...
        table_name: 'managers',
        new_data: { name: form.name, company: form.company }
      });
    }

    // Store for OTP step
    localStorage.setItem('mgr_reg_email', form.email);
    localStorage.setItem('mgr_reg_password', form.password);
  ...
  const verifyOtp = async () => {
    ...
    const password = localStorage.getItem('mgr_reg_password');
