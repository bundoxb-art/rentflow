...
export default function ApartmentAdminLogin() {
  ...
  const login = async () => {
    ...
    if (data?.session) {
      const { data: admin } = await supabase
        .from('apartment_admins').select('*').eq('email', form.email).single();

      if (!admin) {
        await supabase.auth.signOut();
        setError("No Apartment Admin account found.");
      ...
      localStorage.setItem('aa_email', form.email);
      localStorage.setItem('aa_password', form.password);
      setStep(2);
    }
    setLoading(false);
  };
  ...
  const verify = async () => {
    ...
    const email = localStorage.getItem('aa_email');
    const password = localStorage.getItem('aa_password');
    ...
    localStorage.removeItem('aa_password');
    window.location.href = '/apartmentadmin';
  };
    ...
    <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center px-4 font-sans">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <div className="inline-block bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-bold px-4 py-1.5 rounded-full mt-2">
            🏛️ APARTMENT ADMIN PORTAL
          </div>
        </div>

        <div className="bg-[#111827] border border-blue-500/20 rounded-2xl p-8">
          {step === 1 && (
            <>
              <h1 className="text-xl font-extrabold mb-6 text-center">Apartment Admin Login</h1>
              <div className="space-y-4">
                <div>
                  ...
                  <input type="email" value={form.email}
                    ...
                    className="w-full bg-[#0d1117] border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm focus:outline-none focus:border-blue-400 transition" />
                </div>
                <div>
                  ...
                  <div className="relative">
                    <input type={showPassword ? "text" : "password"} value={form.password}
                      ...
                      placeholder="••••••••"
                      className="w-full bg-[#0d1117] border border-white/10 rounded-xl px-4 py-3.5 pr-12 text-white text-sm focus:outline-none focus:border-blue-400 transition" />
                ...
                <button onClick={login} disabled={loading}
                  className="w-full bg-blue-500 text-white font-extrabold py-4 rounded-xl text-sm hover:opacity-90 transition disabled:opacity-70">
          ...
          {step === 2 && (
            <div className="text-center">
              ...
              <div className="flex gap-1.5 justify-center mb-6">
                {otp.map((digit, i) => (
                  <input key={i} id={`otp-${i}`} type="text" inputMode="numeric" maxLength={1} value={digit}
                    ...
                    className={`w-9 h-12 sm:w-11 sm:h-14 text-center text-lg font-extrabold rounded-xl border-2 bg-[#0d1117] text-white transition-all outline-none ${digit ? "border-blue-400" : "border-white/10"} focus:border-blue-400`}
                  />
                ))}
              </div>
              ...
              <button onClick={verify} disabled={loading || otp.join("").replace(/\s/g,'').length < 6}
                className="w-full bg-blue-500 text-white font-extrabold py-4 rounded-xl text-sm hover:opacity-90 transition disabled:opacity-50 mb-4">
